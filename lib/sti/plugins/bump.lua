		...
		for _, layer in ipairs(map.layers) do
			...
			if layer.properties.collidable == true then
				if layer.type == "tilelayer" then
					-- ADDED CUSTOM PATCH TO LET BUMP PLUGIN SUPPORT INFINITE LAYERS
					-- LETS SEE IF THIS HOLDS UP
					-- MAKE PR OF THIS LATER
					for _, chunk in ipairs(layer.chunks or {layer}) do
						local chunkx = (chunk.x or 0) * map.tilewidth
						local chunky = (chunk.y or 0) * map.tileheight
						for y, tiles in ipairs(chunk.data) do
							for x, tile in pairs(tiles) do
									...
									for _, object in ipairs(tile.objectGroup.objects) do
										if object.properties.collidable == true then
											local t = {
												name       = object.name,
												type       = object.type,
												x          = ((x-1) * map.tilewidth  + tile.offset.x + map.offsetx) + object.x + chunkx,
												y          = ((y-1) * map.tileheight + tile.offset.y + map.offsety) + object.y + chunky,
												width      = object.width,
												height     = object.height,
												layer      = layer,
												properties = object.properties
								...
								local t = {
									x          = (x-1) * map.tilewidth  + tile.offset.x + map.offsetx + chunkx,
									y          = (y-1) * map.tileheight + tile.offset.y + map.offsety + chunky,
									width      = tile.width,
									height     = tile.height,
									layer      = layer,
									type       = tile.type,
